       ...
       public class ForForms
        {
            ...
            //
            public void Save_with_path_to_file(string theText, bool toggle = false)
            {
                SaveFileDialog Saving = new SaveFileDialog();
                string path = "";
                if (Saving.ShowDialog() == true)
                {
                    ...
                    {
                        path = Saving.FileName;
                        InfoBank.path_to_file = Saving.FileName;
                        using (FileStream file1 = new FileStream(path, FileMode.Create, FileAccess.ReadWrite))
                        {
                            StreamWriter writter = new StreamWriter(file1);
                            writter.WriteLine(theText);
            ...
            public string open_with_path()
            {
                ...
                try
                {
                    ...
                    {
                        path = open.FileName;
                        InfoBank.path_to_file = open.FileName ;
                    }
                }
                catch
                {
                ...
                try
                {
                    ...
                    if (File.Exists("options.opt"))
                    {
                        ...
                        if (redactinglines[5] != "fine")
                        {
                            ...
                            redactinglines[1] = TCP_SOCKET.host;
                            redactinglines[2] = TCP_SOCKET.port.ToString();
                            redactinglines[3] = TCP_SOCKET.userName;
